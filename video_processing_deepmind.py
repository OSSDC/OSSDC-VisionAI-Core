...
pathToProject='../deepmind-research'
...
def init_model(transform):
  global variant,params,imgLoaded
  if transform == "nfnets":
    # Load F0 weights
    ...
    imgLoaded =  cv2.imread('peppers.jpg')
  ...
  return (fwd,imsize),None

def process_image(transform,processing_model,img):
  global imagenet_classlist,imgLoaded
  tracks = []
  try:
    if transform == "nfnets":
      (fwd,imsize) = processing_model
      # img = imgLoaded #override to test performance
      im = img
      # Resize and crop to variant test size
      im = cv2.resize(im,(imsize + 32, imsize + 32))
      # im = im[16, 16, 16+imsize, 16+imsize]
      im = im[16:16+imsize, 16:16+imsize]
      ...
      label = imagenet_classlist[int(logits.argmax())]
      cv2.putText(img, "Pred: "+label, (10,150), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,255,0), 2)
